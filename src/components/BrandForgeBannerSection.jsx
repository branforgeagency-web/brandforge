...
/* ───────────────────────────────────────────────────────────────────────────
   BRANDFORGE HYPERSPEED BANNER SECTION
   3D WebGL Speed Trails with PostProcessing Bloom & Centered Logo Overlay.
   ─────────────────────────────────────────────────────────────────────────── */
...
const defaultOptions = {
  ...
  colors: {
    ...
    rightCars: [0xffffff, 0xf8fafc, 0xffa048],
    sticks: 0xef4136,
  },
};
...
export default function BrandForgeBannerSection({ onOpenModal }) {
  ...
  return (
    <section className="bf-banner-root">
      <style>{styles}</style>

      {/* 3D HYPERSPEED LIGHT TRAILS CANVAS */}
      <div id="lights" ref={hyperspeedRef} />

      {/* COSMETIC DARK OVERLAY VIGNETTE */}
      <div className="bf-banner-vignette" />

      {/* CENTERED BRANDFORGE LOGO OVERLAY */}
      <div className="bf-banner-content">
        <motion.div
          className="bf-banner-logo-center"
          initial={{ opacity: 0, scale: 0.88 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 1.0, ease: [0.16, 1, 0.3, 1] }}
        >
          {/* CENTERED NEW 3D LOGO ASSET */}
          <img
            src="/brandforge-logo.png"
            alt="BrandForge 3D Logo"
...
const styles = `
  ...
  .bf-banner-logo {
    height: auto;
    max-height: clamp(65px, 10vw, 130px);
    width: 100%;
    max-width: 720px;
    object-fit: contain;
  ...
  .bf-banner-title span {
    display: block;
    color: #EF4136;
    text-shadow: 0 0 30px rgba(239, 65, 54, 0.35);
  }
  ...
  .bf-banner-btn.primary:hover {
    background: #D9382E;
    transform: translateY(-2px);
    box-shadow: 0 16px 42px rgba(239, 65, 54, 0.6);
  }

  .bf-banner-btn.secondary {
